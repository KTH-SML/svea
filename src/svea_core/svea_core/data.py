...
from svea_core.simulators.viz_utils import (
    publish_path,
    publish_3Dcar,
    publish_target,
    plot_car,
...
class BasicDataHandler(object):
    """ Handler for the basic data-related tasks, including logging, plotting,            # self.actuation_subscriber = rospy.Subscriber(
    ...
    def __init__(self, Node=Node, vehicle_name=''):
        ...
        self.start_time = rclpy.clock.Clock().now().to_msg()
    ...
    def log_state(self, state):
        ...
        self.t.append(state.time_stamp.sec-self.start_time.sec)
...
class RVIZPathHandler(TrajDataHandler):
    ...
    def __init__(self, Node=Node, vehicle_name='', traj_x=[], traj_y=[]):
