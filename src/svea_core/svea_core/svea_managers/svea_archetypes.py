...
from svea_core.sensors import Lidar
from svea_core.data import BasicDataHandler
...
class SVEAManager(object):
    ...
    def __init__(self, Node=Node, localizer=None, controller=None,
        ...
        self.localizer = localizer(self.node,vehicle_name)
        self.controller = controller(vehicle_name)
        self.actuation = actuation(self.node, vehicle_name)
        self.data_handler = data_handler(self.node, vehicle_name)
    ...
    def wait_for_state(self):
        ...
        """
        time = rclpy.clock.Clock().now().to_msg()
        ...
        wait = rclpy.clock.Clock().now().to_msg() - time
    ...
    def send_control(self, steering, velocity, transmission=-1):
        ...
        self.data_handler.log_ctrl(steering, velocity, rclpy.clock.Clock().now().to_msg())
