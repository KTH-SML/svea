...
class VehicleStateTest(unittest.TestCase):
    ...
    og_kw_dict = dict(
        ...
        covariance=None,
        time_stamp=rclpy.clock.Clock().now().to_msg(),
        # time_stamp=rclpy.Time.from_sec(0)
        )
    ...
    def populate_odom_msg(self, msg):
        ...
        # header.stamp = rclpy.Time.from_sec(3.0)
    ...
    def test_set_state(self):
        ...
        state_msg.header.stamp = rclpy.clock.Clock().now().to_msg()
        # state_msg.header.stamp = rclpy.Time.from_sec(0.99)
