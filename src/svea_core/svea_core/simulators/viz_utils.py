...
def publish_3Dcar(polygon_publisher, pose_publisher, x, y, yaw):
    ...
    :type polygon_publisher: rclpy.topics.Publisher
    :param pose_publisher: ROS publisher to broadcast fake pose with
    :type pose_publisher: rclpy.topics.Publisher
    :param x: current x position of car in [m]
    :type x: float
    :param y: current y position of car in [m]
    :type y: float
    ...
    for i, (px, py) in enumerate(zip(xs, ys)):
        ...
        point.x = px
        point.y = py
        point.z = chassis_height if i < 5 else top_height
        pts.append(point)

    # build polygon
    car_poly = PolygonStamped()
    car_poly.header.stamp = rclpy.clock.Clock().now().to_msg()
    ...
    car_pose.header.stamp = rclpy.clock.Clock().now().to_msg()
...
def lists_to_pose_stampeds(x_list, y_list, yaw_list=None, t_list=None):
    ...
    for i in range(len(x_list)):
        ...
        if not t_list is None:
            t = t_list[i]
            curr_pose.header.stamp = Time(secs = t)
        else:
            curr_pose.header.stamp = rclpy.clock.Clock().now().to_msg()
...
def publish_target(target_publisher, x, y):
    ...
    :type target_publisher: rclpy.topics.Publisher
    """
    target_pt = PointStamped()
    target_pt.header.stamp = rclpy.clock.Clock().now().to_msg()
...
def publish_pose_array(poses_publisher, x_list, y_list, yaw_list):
    ...
    :type poses_publisher: rclpy.topics.Publisher
    ...
    pose_array.header.stamp = rclpy.clock.Clock().now().to_msg()
...
def publish_lidar_points(point_cloud_publisher, points):
    ...
    :param point_cloud_publisher: ROS publisher for broadcasting
                                  pointcloud with
    :type point_cloud_publisher: rclpy.topics.Publisher
...
def publish_lidar_rays(rays_publisher, lidar_pos, points):
    ...
    :type rays_publisher: rclpy.topics.Publisher
    ...
    """
    marker_msg = Marker()
    marker_msg.header.stamp = rclpy.clock.Clock().now().to_msg()
...
def publish_edges(edges_publisher, edges):
    ...
    :type edges_publisher: rclpy.topics.Publisher
