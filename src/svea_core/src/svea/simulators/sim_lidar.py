...
class SimLidar(object):
    ...
    def __init__(self, vehicle_name=''):
        ...
        self._scan_msg.header.stamp = rclpy.Time.now()
    ...
    def _init_and_spin_ros(self):
        rclpy.loginfo("Starting Lidar Simulation Node: \n"
                      + str(self))
        self._start_publish()
        rclpy.loginfo("{} Simulated Lidar successfully initialized".format(
            self.vehicle_name))
    ...
    def _start_publish(self):
        ...
        self._viz_points_pub = rclpy.Publisher(self._viz_points_topic,
                                               PointCloud,
                                               queue_size=1)
        self._viz_rays_pub = rclpy.Publisher(self._viz_rays_topic,
                                             Marker,
                                             queue_size=1)
        self._viz_edges_pub = rclpy.Publisher(self._viz_edges_topic,
                                              Marker,
                                              queue_size=1)

    def _start_simulation(self):
        rate = rclpy.Rate(1/self.SCAN_TIME)
    ...
    def obstacles(self):
        ...
        obstacles_from_param = rclpy.get_param(obstacles_param, None)
