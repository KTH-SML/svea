...
class SimSVEA(object):
    ...
    def __init__(self,
        ...
        self._last_pub_time = rclpy.get_time()
        ...
        self._last_ctrl_time = rclpy.get_time()
    ...
    def _init_and_spin_ros(self):
        rclpy.loginfo("Starting Simulation Node: \n"
                      + str(self))
        self._collect_srvs()
        self._start_publish()
        rclpy.loginfo("{} Simulation successfully initialized".format(
            self.vehicle_name))
        self._start_listen()
        rclpy.spin()
    ...
    def _start_listen(self):
        rclpy.Subscriber(self._request_topic,
                         lli_ctrl,
                         self._update_ctrl_request)
        rclpy.Subscriber(self._emergency_topic,
                         lli_emergency,
    ...
    def toggle_pause_simulation(self):
        ...
        rclpy.loginfo("Simulation is now {0}".format(status))

    def _start_simulation(self):
        rate = rclpy.Rate(1/self.dt)
        while not rclpy.is_shutdown():
            curr_time = rclpy.get_time()
