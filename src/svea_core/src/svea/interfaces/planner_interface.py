...
def load_param(name, value=None):
    if value is None:
        assert rclpy.has_param(name), f'Missing parameter "{name}"'
    return rclpy.get_param(name, value)
...
class PlannerInterface(object):
    ...
    def __init__(self):
        ...
        self.init_gridmap_subscribers()
        rclpy.loginfo("Planning interface correctly initialized")
        
    ## GRIDMAP INTERFACE ##
    def init_gridmap_subscribers(self):
        self._gridmap_sub = rclpy.Subscriber(self._map_topic, OccupancyGrid, self._gridmap_cb, queue_size=1)
    ...
    def publish_map_internal_representation(self):
        ...
        map_msg.header.stamp = rclpy.Time.now()
    ...
    def publish_rviz_path(self):
        if self._pose_path:
            ...
            self._rviz_path.header.stamp = rclpy.Time.now()
    ...
    def get_path_from_topic(self):
        msg = rclpy.wait_for_message('/smooth_path', Path, timeout=None)
        path = []
        for p in msg.poses:
