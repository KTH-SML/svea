...
class Lidar():
    ...
    def _init_and_spin_ros(self):
        rclpy.logging.info("Starting Lidar Interface Node: \n" + str(self))
        self._collect_srvs()
        self._start_listen()
    ...
    def _start_listen(self):
        Node.create_subscription(self, LaserScan, 'scan', self._read_scan,)
        # rospy.Subscriber('scan', LaserScan, self._read_scan,
        #                  tcp_nodelay=True)
        rclpy.logging.get_logger().info("Lidar Interface successfully initialized")
...
class WheelEncoder():
    ...
    def _init_and_spin_ros(self):
        rclpy.logging.get_logger().info('Starting wheel encoder Interface for'
                      + self.vehicle_name)
        self._collect_srvs()
        self._start_listen()
        rclpy.logging.get_logger().info('Succesfully initiated wheel encoder Interface for'
                        + self.vehicle_name)
    ...
    def _start_listen(self):
        self.emcoder_subscriber = Node.create_subscription(
            self,
            lli_encoder,
            self.encoder_topic,
            self._process_encoder_data)
        if self.direction_topic:
            self.actuation_subscriber = Node.create_subscription(
                self,
