...
class LocalizationInterface:
    ...
    def __init__(
        ...
        rclpy.on_shutdown(self._shutdown_callback)
    ...
    def _init_and_spin_ros(self):
        rclpy.loginfo("Starting Localization Interface Node for "
                      + self.vehicle_name)
        self.node_name = 'localization_node'
        self._start_listen()
        self.is_ready = self._wait_until_ready()
        if not self.is_ready:
            rclpy.logwarn("Localization not responding during start of "
                          "Localization Interface. Setting ready anyway.")
        self.is_ready = True
        rclpy.loginfo("{} Localization Interface successfully initialized"
                      .format(self.vehicle_name))

        rclpy.spin()

    def _start_listen(self):
        rclpy.Subscriber(self._state_topic,
                         VehicleStateMsg,
